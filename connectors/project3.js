...
function execute(n, m) {
	...
	let pyshell = new PythonShell("givens_rotation.py", options);
...
exeBtn.addEventListener("click", function () {
	var n = inputN.value;
	if (!n || n <= 0) alert("Given n is invalid!");
...
${beautify(JSON.stringify(result.A), options)}

*********************************

Q
${beautify(JSON.stringify(result.Q), options)}

*********************************

R
${beautify(JSON.stringify(result.R), options)}
                `;
